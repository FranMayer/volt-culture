 ...
 * Transcripción literal de merge() en legacy/js/cart-sync.js:48-59: usa
 * Math.max en vez de sumar para evitar duplicación cuando local y Firestore
 * ya están sincronizados (ej: refresh de página). Solo agrega items que
 * existan únicamente en local (sesión anónima previa). Usada por
 * lib/cart/sync.ts#loadAndMerge (merge en login).
 */
export function mergeCartItems(firestoreItems, localItems) {
