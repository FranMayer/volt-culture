/**
 * rate-limit.ts — Rate limit por IP (Upstash Redis). Port literal de api/_rate-limit.js.
 */
...
export type RateLimitRouteKey = 'create-preference' | 'welcome-email' | 'public-default';

const FAIL_CLOSED_ROUTES = new Set(['create-preference', 'welcome-email']);

const LIMIT_CONFIG: Record<string, { limit: number; window: `${number} ${'m' | 'h' | 's'}` }> = {
...
function getLimiter(routeKey: string): Ratelimit | null {
...
export async function applyRateLimit(
    req: NextApiRequest,
    res: NextApiResponse,
    routeKey: RateLimitRouteKey = 'public-default'
): Promise<boolean> {
