/**
 * shipping-email.ts — Port literal de api/_shipping-email.js.
 * Único cambio de comportamiento: importa SHIPPING_CONFIG desde lib/shipping-config
 * (el original importaba '../js/shipping-config.js', ruta que dejó de existir cuando
 * F0 movió el frontend viejo a legacy/ — ese import roto es un problema de api/_shipping-email.js
 * en sí, fuera de alcance de F2a; ver nota en el reporte al orquestador).
 */
import { SHIPPING_CONFIG } from '../shipping-config';

function formatShippingCostArs(amount: number) {
    return Number(amount).toLocaleString('es-AR');
}

/** Textos para checkout actual (shipping.type: cordoba | andreani). */
export const SHIPPING_TYPE_DISPLAY: Record<string, string> = {
...
function escapeHtmlAttr(text: unknown) {
    return String(text)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
 ...
 */
export function resolveShippingType(shipping: any): 'cordoba' | 'andreani' | null {
...
function resolveLegacyShippingMethod(shipping: any): string | null {
    if (!shipping || typeof shipping !== 'object') return null;
...
export function formatShippingBlockClientHtml(orderData: any): string {
...
export function formatShippingBlockAdminHtml(orderData: any): string {
    const s = orderData.shipping;
    const type = resolveShippingType(s);
    if (type) {
        const label = escapeHtmlAttr(SHIPPING_TYPE_DISPLAY[type]);
